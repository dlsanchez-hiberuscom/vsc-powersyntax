...
import type { ObjectExplorerLoadMoreNode } from './objectExplorerProjectionModel';
...
const READ_ONLY_EXPORT_PROBE_TIMEOUT_MS = 5000;
...
function ensureObjectExplorerController(): PowerBuilderObjectExplorerController {
  if (!objectExplorerController) {
    ...
    objectExplorerController = new PowerBuilderObjectExplorerController(
      (request: ApiObjectExplorerProjectionRequest) => executeServerCommand<ApiObjectExplorerProjectionPage>(
...
function ensureCommandsRegistered(context: vscode.ExtensionContext): void {
  ...
  registerClientCommands(context, {
    ...
    loadMoreObjectExplorerNodes: async (target?: unknown) => {
      await ensureObjectExplorerController().loadMore(target as ObjectExplorerLoadMoreNode | undefined);
    },
...
function createPublicApi(): VscPowerSyntaxApi {
  ...
  const api: VscPowerSyntaxApi = {
    ...
    async getAiTaskContextBundle(request: ApiAiTaskContextBundleRequest = {}): Promise<ApiAiTaskContextBundle> {
      ...
      const executionPlan = buildAiTaskContextExecutionPlan(resolvedRequest);
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'workspaceCheck')) {
        sectionWork.push((async () => {
          try {
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'objectCheck')) {
        sectionWork.push((async () => {
          try {
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'currentObjectContext') && resolvedSource.uri) {
        sectionWork.push((async () => {
          try {
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'safeEditPlan') && resolvedSource.uri) {
        sectionWork.push((async () => {
          try {
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'dependencyGraph') && (resolvedSource.uri || resolvedSource.objectName)) {
        sectionWork.push((async () => {
          try {
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'diagnosticExplanations') && resolvedSource.uri) {
        sectionWork.push((async () => {
          try {
      ...
      if (shouldExecuteAiTaskContextSection(executionPlan, 'systemSymbolExplanations')) {
        sectionWork.push((async () => {
          try {
...
async function exportSemanticReproPack(options?: SemanticReproPackCommandOptions): Promise<SemanticReproPackExportResult | undefined> {
  ...
  const focusCharacter = editor.selection.active.character;
  const [currentObjectContext, impactAnalysis, safeEditPlan, workspaceManifest, serverStats] = await Promise.all([
    withBoundedReadOnlyExportProbe(
      'semantic-repro/current-object-context',
      ...
      (reason) => createUnavailableCurrentObjectContext(focusUri, reason),
    ),
    withBoundedReadOnlyExportProbe(
      'semantic-repro/impact-analysis',
      () => publicApiSingleton.analyzeImpact({
        uri: focusUri,
        line: focusLine,
        character: focusCharacter,
        maxSafeReferences: 50,
      }),
      createUnavailableImpactAnalysis,
    ),
    withBoundedReadOnlyExportProbe(
      'semantic-repro/safe-edit-plan',
      () => publicApiSingleton.generateSafeEditPlan({
        uri: focusUri,
        line: focusLine,
        character: focusCharacter,
        maxSafeReferences: 50,
      }),
      createUnavailableSafeEditPlan,
    ),
    withBoundedReadOnlyExportProbe(
      'semantic-repro/workspace-manifest',
      () => publicApiSingleton.getSemanticWorkspaceManifest({
        maxObjects: 200,
        maxSymbols: 400,
      }),
      createDegradedSemanticWorkspaceManifest,
    ),
    withBoundedReadOnlyExportProbe(
      'semantic-repro/server-stats',
      () => publicApiSingleton.getServerStats(),
...
async function exportSupportBundle(options?: SupportBundleCommandOptions): Promise<SupportBundleExportResult | undefined> {
  ...
  const activePowerBuilderEditor = activeEditorInWorkspace?.document.languageId === LANGUAGE_ID
    ...
    : undefined;
  const [workspaceManifest, serverStats, codeMetrics, technicalDebtReport, workspaceMigrationAssistant, currentObjectContext] = await Promise.all([
    withBoundedReadOnlyExportProbe(
      'support-bundle/workspace-manifest',
      () => fetchSupportBundleWorkspaceManifest(),
      createDegradedSemanticWorkspaceManifest,
    ),
    withBoundedReadOnlyExportProbe(
      'support-bundle/server-stats',
      () => publicApiSingleton.getServerStats(),
      createDegradedServerStats,
    ),
    withOptionalBoundedReadOnlyExportProbe(
      'support-bundle/code-metrics',
      () => publicApiSingleton.getPowerBuilderCodeMetrics({
        maxObjects: 80,
      }),
    ),
    withOptionalBoundedReadOnlyExportProbe(
      'support-bundle/technical-debt-report',
    ...
    withOptionalBoundedReadOnlyExportProbe(
      'support-bundle/workspace-migration-assistant',
    ...
    activePowerBuilderEditor
      ? withOptionalBoundedReadOnlyExportProbe(
        'support-bundle/current-object-context',
        () => publicApiSingleton.getCurrentObjectContext({
...
async function exportHealthReport(options?: HealthReportCommandOptions): Promise<HealthReportExportResult | undefined> {
  ...
  const manifest = await withBoundedReadOnlyExportProbe(
    'health-report/workspace-manifest',
    () => publicApiSingleton.getSemanticWorkspaceManifest({
      maxObjects: 200,
      maxSymbols: 400,
    }).then((result) => clonePlainData(result)),
    createDegradedSemanticWorkspaceManifest,
  );
  ...
  return {
    ...
    manifestUri: joinUriPath(destinationUri, 'semantic-workspace-manifest.json').toString(),
  };
}
...
async function withBoundedReadOnlyExportProbe<T>(
  label: string,
  operation: () => Thenable<T> | Promise<T>,
  fallback: (reason: string) => T,
): Promise<T> {
  let timedOut = false;
  const timeoutReason = `timeout:${label}`;
  const timeoutFallback = new Promise<T>((resolve) => {
    setTimeout(() => {
      timedOut = true;
      resolve(fallback(timeoutReason));
    }, READ_ONLY_EXPORT_PROBE_TIMEOUT_MS);
  });

  try {
    const result = await Promise.race([Promise.resolve().then(operation), timeoutFallback]);
    if (timedOut) {
      outputChannel?.appendLine(`[ReadOnlyExport] ${label} excedio ${READ_ONLY_EXPORT_PROBE_TIMEOUT_MS}ms; degradando resultado.`);
    }
    return result;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    outputChannel?.appendLine(`[ReadOnlyExport] ${label} fallo: ${message}; degradando resultado.`);
    return fallback(`error:${label}:${message}`);
  }
}

async function withOptionalBoundedReadOnlyExportProbe<T>(
  label: string,
  operation: () => Thenable<T> | Promise<T>,
): Promise<T | undefined> {
  return withBoundedReadOnlyExportProbe<T | undefined>(
    label,
    operation,
    () => undefined,
  );
...
function createUnavailableCurrentObjectContext(uri: string, reason: string): ApiCurrentObjectContext {
  return {
    available: false,
    reason,
    uri,
...
function createUnavailableImpactAnalysis(reason: string): ApiImpactAnalysis {
  return {
    available: false,
    reason,
    safeReferences: [],
...
function createUnavailableSafeEditPlan(reason: string): ApiSafeEditPlan {
  return {
    available: false,
    blocked: true,
    reason,
...
function createDegradedSemanticWorkspaceManifest(reason: string): ApiSemanticWorkspaceManifest {
  return {
    schemaVersion: '1.0.0',
...
function createDegradedServerStats(reason: string): ApiServerStats {
  return {
    readiness: {
      state: 'degraded',
      detail: reason,
