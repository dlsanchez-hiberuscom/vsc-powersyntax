...
const SPECIAL_SUMMARIES = new Map<string, string>([
    ['CurrentRow', 'Returns the current row evaluated by the DataWindow expression.'],
    ['Describe', 'Retrieves a Describe/Modify property from a DataWindow expression.'],
    ['GetRow', 'Returns the row associated with the current evaluation context of the DataWindow expression.'],
    ['GetText', 'Returns the active text of the paint context in a DataWindow expression.'],
    ['If', 'Evaluates a condition and returns one of two values within a DataWindow expression.'],
    ['LookUpDisplay', 'Returns the display value of a DropDownDataWindow within a DataWindow expression.'],
    ['PageCount', 'Returns the total number of pages generated by the DataWindow.'],
    ['RowCount', 'Returns the number of visible rows for the current evaluation of the DataWindow expression.'],
    ['Sum', 'Aggregates values of an expression or column over the set of rows in the DataWindow.'],
]);
...
function buildDataWindowExpressionFunctionSummary(
    ...
    switch (category) {
        case 'Aggregation':
            return 'Official aggregation function available in DataWindow expressions.';
        case 'Context':
            return 'Official context or state function available in DataWindow expressions.';
        case 'Date and Time':
            return 'Official date and time function available in DataWindow expressions.';
        case 'Mathematical':
            return 'Official numeric function available in DataWindow expressions.';
        case 'Text':
            return 'Official text function available in DataWindow expressions.';
        case 'Visual':
            return 'Official visual or layout function available in DataWindow expressions.';
    }
}
...
function defineDataWindowExpressionFunctionEntries(
    ...
    return descriptors.map(descriptor => {
        ...
        return dataWindowExpressionFunction({
            ...
            manualOverlay: {
                mode: 'override',
                reason: 'Enforces curated documentation for DataWindow expression functions.',
                evidence: [`manual-core:datawindow-expression-functions:callable:datawindow-expression:global:${descriptor.name.toLowerCase()}`],
            },
