...
function mapToSemanticFacts(facts: SymbolFact[], uri: string, sourceOrigin: ReturnType<typeof inferSourceOrigin>): Fact[] {
  ...
  for (const f of facts) {
    ...
    factMap.set(dedupKey, {
      ...
      externalCallableKind: f.externalCallableKind,
      ...
      if (section.kind === 'prototypes') {
        ...
        if (external) {
          facts.push({
            ...
            isExternal: true,
            externalCallableKind: external.externalCallableKind,
            externalLibraryName: external.library,
            externalAlias: external.alias,
            externalDependencyKind: external.library ? classifyExternalLibrary(external.library) : undefined,
            line: i,
