...
function buildDiagnosticsByObject(
  ...
  for (const project of diagnosticsSummary.projects) {
    for (const objectNode of project.objects) {
      const lifecycleWarnings = Object.entries(objectNode.byCode).reduce((count, [codeKey, total]) => {
        return count + (isLifecycleDiagnosticCode(normalizeDiagnosticCode(codeKey).toLowerCase()) ? total : 0);
      }, 0);
