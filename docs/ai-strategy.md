...
- Encapsular primero el contexto en surfaces compactas read-only (`workspace-check`, `object-check`, `explain-diagnostic`, `explain-system-symbol`, `ai-task-context-bundle`) con budgets explícitos, receipts de paginación y `reasonCodes` machine-readable junto a `omissions` honestas antes de abrir prompts o participants más ricos.
