...
- `B327` queda ya cerrado: `generated/dataWindowConstants.generated.ts` proyecta `DWBuffer`, `Primary!`, `Delete!`, `Filter!` y el resto de constantes oficiales DataWindow sobre el dominio `datawindow-constants` sin duplicar el rail enumerado global, `queryService.ts` / `SystemCatalog.ts` exponen queries owner-scoped para ese dominio y `completion.ts` / `signatureHelp.ts` lo consumen sólo cuando el owner member ya es DataWindow/DataStore/DataWindowChild.
