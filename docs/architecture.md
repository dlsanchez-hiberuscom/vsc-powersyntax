...
- `B327` queda ya cerrado: `generated/dataWindowConstants.generated.ts` materializa `datawindow-constants` como proyección oficial DataWindow sobre el rail enumerado ya existente, `queryService.ts` / `SystemCatalog.ts` exponen queries owner-scoped para ese dominio y `completion.ts` / `signatureHelp.ts` sólo lo consumen cuando el owner ya está resuelto como `datawindow|datastore|datawindowchild`, manteniendo `Describe/Modify/Object` sobre `datawindow-properties` y sin abrir listas locales nuevas.
- `B342` queda ya cerrado: `linkedEditing.ts` reutiliza `queryContext` + `references` para publicar linked editing sólo sobre `Local` y `Argumento` con resolución semántica única, `featureHandlers.ts` lo sirve con los mismos readiness/confidence gates de rename y el servidor anuncia la capacidad LSP sin reintroducir provider host ni un rail semántico paralelo de `plugin_old`.
...
- `B340` deja además `tooling-symbols` como dominio manual explícito bajo el namespace `powerbuilder-tooling` para ORCA/PBAutoBuild, env vars y settings de tooling, pero `resolveLanguageSymbol()` lo excluye del hot path interactivo para que ese vocabulario solo alimente surfaces de docs/health/build y no compita con PowerScript/DataWindow;
...
- `B335` consolida ese contrato como dashboard/gate reproducible: `workspaceCheckCatalogSummary.ts` reconsume `buildCatalogConsistencyReport()`, cruza el hot path resuelto con el registro bruto para detectar candidate leakage auditable sin contaminar el runtime y publica `adrCompliance`; `workspaceCheckReport.ts` eleva ese estado a findings/status del `workspace-check`; `npm run report:catalog-consistency` serializa JSON/Markdown determinista en `artifacts/catalog/`; y `npm run verify:catalog-coverage` endurece en CI el drift de `officialCoverage` sin mover ese audit al hot path interactivo;
...
- `workspace-check` read-only servido por API pública/tool bridge/comandos Markdown, compuesto cliente-side sobre `server-stats`, `semanticWorkspaceManifest`, diagnostics/health runtime, settings governance, `workspaceMigrationAssistant` y un summary ligero de catálogo memoizado, con modos `quick/full/catalog/diagnostics/upgrade` para revisar también compatibilidad de upgrade (cache policy/runtime persistente, settings legacy, snapshots/API version y artefactos locales) sin abrir un motor paralelo de validación;
...
- `B284` deja además `explain-semantic-query` como report read-only servido por API pública/tool bridge/comando Markdown, compuesto server-side sobre `queryContext` + `ResolvedTargetInfo` + `queryTrace`, con fases legibles, candidates/discards/winner, `confidence`/`sourceOrigin`/coste aproximado y sin abrir otro motor de resolución ni duplicar la semántica del hot path;
- `ai-task-context-bundle` read-only servido por API pública/tool bridge/comando oculto, compuesto cliente-side sobre `workspace-check`, `object-check`, `currentObjectContext`, `safeEditPlan`, `dependencyGraph`, `explain-diagnostic` y `explain-system-symbol`, con prioridades por `intent`, budget conservador, `reasonCodes`/receipt de paginación machine-readable y `omissions` explícitas en vez de reabrir contexto masivo o duplicar semántica;
- `task-execution-dry-run` read-only servido por API pública/tool bridge, compuesto cliente-side sobre `safeEditPlan` e `impactAnalysis` para exponer plan, impacto, archivos, tests, docs y bloqueos antes de cualquier rail write-enabled;
- `task-replay-bundle` read-only servido por API pública/tool bridge, compuesto cliente-side sobre manifests versionados de `semantic-repro-pack` y `support-bundle` para rehidratar `minimalContext`, `referencedFiles`, `suggestedCommands` y `recommendedContractId` sin exigir el repo completo;
...
- workflows write-enabled `applySpecDrivenPblUpdate` / `applySpecDrivenPblUpdateBatch` servidos por API pública/LSP, reutilizando `safeEditPlan`, export ORCA fresco, edits explícitos sobre staging y el mismo rail de import con backup/ledger/journal, y devolviendo `validationReceipt` en vez de abrir un postprocesado legacy paralelo;
...
- policy read-only compartida de conflicto entre símbolos reales del workspace y framework knowledge packs curados, servida por `queryApiSymbols`, `currentObjectContext`, `impactAnalysis`, `safeEditPlan` y `object-check` a partir de owner types curados + `sourceOrigin` autoritativo, degradando packs a contexto advisory sin alterar el winner real del query engine;
...
- registro único `queryScopePolicy` por consumer semántico, declarando `maxScope`, `budgetMs`, `resultCap`, readiness, confidence, fallback y allowances `staging/generated/external`; `queryContext`, `semanticQueryService`, `signatureHelp`, `completion`, `hover`, `definition`, `references`, `rename`, `currentObjectContext`, `impactAnalysis`, `diagnostics` y `referenceSourcePool` ya consumen ese contrato para que el winner path y sus surfaces derivadas filtren `orca-staging/generated/external` según el consumer sin ensanchar a `workspace` ni reintroducir guards locales por feature;
- registro único `backpressurePolicy` por workload runtime, declarando `lane`, `throttledByLatency` y `preemptible`; `TaskScheduler`, `diagnosticScheduler`, `managedRuntimeWorkloads.ts`, `managedBuildWorkloads.ts`, `server.ts` y las surfaces visibles de stats/health ya consumen ese contrato para enrutar `diagnostics`, `near-context`, `background-indexing`, `export-reporting`, `build`, `legacy-orca` y `maintenance` sobre el mismo scheduler, preservando `build/legacy-orca` una vez arrancan y manteniendo preemptibles los carriles read-only/background;
...
- lifecycle PowerBuilder integrado en surfaces visibles: `hierarchyInspection` reconstruye create/destroy desde snapshot semántico, hover explica `constructor/destructor` desde el mismo modelo, diagnostics emite warnings suaves (`missing-super-*`, `missing-trigger-*`, `unresolved-*`) y desde `B310` `code metrics`/`technical-debt-report` los proyectan como `lifecycleWarnings` y hotspot `lifecycle-risk` sin abrir un segundo motor.
...
- Refactor arquitectónico de entrypoints: `extension.ts` y `server.ts` siguen siendo los hotspots principales; `server.ts` ya quedó reducido a bootstrap + composición de controladores runtime, mientras lifecycle/document/features/commands viven en `handlers/lifecycleHandlers.ts`, `handlers/documentHandlers.ts`, `handlers/featureHandlers.ts`, `handlers/buildCommandHandlers.ts`, `handlers/reportCommandHandlers.ts` y `handlers/runtimeCommandHandlers.ts`. La orquestación local de persistencia/readiness/workloads se delega ya en `cache/semanticCacheRuntimeController.ts`, `runtime/runtimeProgressController.ts`, `runtime/managedRuntimeWorkloads.ts` y `runtime/managedBuildWorkloads.ts`, y el LRU de CodeLens sigue en `features/codeLensResultCache.ts` sin reabrir un segundo centro de decisión runtime.
- el runtime distribuido de la extensión se publica ya desde `dist/client/extension.js` y `dist/server/server.js`; `out/server/server.js` queda como fallback exclusivo de `Development`, no como dependencia del VSIX productivo.
