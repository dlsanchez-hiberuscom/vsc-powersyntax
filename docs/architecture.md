...
- `B335` consolida ese contrato como dashboard/gate reproducible: `workspaceCheckCatalogSummary.ts` reconsume `buildCatalogConsistencyReport()` y la merge policy real del query layer para publicar `adrCompliance`, `workspaceCheckReport.ts` eleva ese estado a findings/status del `workspace-check` y `npm run report:catalog-consistency` serializa JSON/Markdown determinista en `artifacts/catalog/` sin mover ese audit al hot path interactivo;
