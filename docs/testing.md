...
- `test/server/unit/workspaceCheckCatalogSummary.test.ts`, `workspaceCheckReport.test.ts`, `catalogConsistency.test.ts`, `catalogAdoptionDecision.test.ts`, `catalogProvenanceAudit.test.ts` y `systemCatalogQueryHardening.test.ts` fijan `B335`: el `workspace-check` reconsume el consistency report real, proyecta `adrCompliance` sobre el catálogo vivo, mantiene `generated-primary-with-manual-overlays` como policy contractual y `npm run report:catalog-consistency` queda validado sobre el mismo baseline generated-primary sin abrir otro rail semántico.
