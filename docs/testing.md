...
La smoke `test/smoke/extension.test.ts` fija además `B376`: warm-up mínimo del workspace, ejecución real del tool `workspace-check` y apertura del comando `PowerSyntax: Check Workspace` con Markdown AI-readable sin introducir un motor paralelo ni writes.
...
- `test/server/unit/aiTaskContextBundle.test.ts`, junto con `test/server/unit/publicApi.test.ts` y `test/smoke/extension.test.ts`, fija `B381`: bundle read-only estable por `intent`, budgets bajos reales, degradación a bundle mínimo, foco ausente honesto y wiring contractual del tool/comando sin duplicar semántica ni cargar datasets completos.
