...
Para el cruce entre capas implementadas, flujos reales, owners y suites que fijan esos contratos, ver también `docs/architecture-implementation-map.md`.
...
Esta es la matriz oficial para cerrar specs y cambios de arquitectura. Antes de marcar un bloque como cerrado, el agente debe escoger el carril mínimo que cubre el riesgo tocado y registrar el comando exacto ejecutado.

| Lane | Comando | Valida | Coste esperado | Cuándo ejecutarlo |
| --- | --- | --- | --- | --- |
| Build/test compile | `npm run build:test` | Cliente, servidor, bundle y TypeScript de tests sin `out/` obsoleto. | Medio | Siempre que haya cambios TS, tests, packaging o wiring. |
| Unit | `npm run test:unit` | Core semántico, caches, presentation, guards unitarios y regresiones deterministas. | Medio | Cambios en `src/server/**`, `src/shared/**`, `test/server/unit/**` o herramientas unit-tested. |
| Integration/LSP | `npm run test:integration` | Contratos de integración del servidor y flujos LSP que no requieren VS Code UI completa. | Medio-alto | Cambios de handlers LSP, lifecycle de servidor, workspace o protocolos. |
| VS Code smoke | `npm run test:smoke` | Activación real, contribution points, comandos principales y wiring de la extension development host. | Alto | Cambios en cliente VS Code, `package.json`, comandos, settings, Tree Views o smoke fixtures. |
| Full standard | `npm test` | Build completo + smoke + unit + integration. | Alto | Cierre de bloque, PRs de alcance transversal o cambios que cruzan cliente/servidor/docs/tests. |
| Architecture rapid | `npm run test:architecture:rapid` | Fitness functions rápidas, smoke/performance etiquetada y boundaries críticas. | Medio-alto | Refactors de capas, imports, owners, hot path, DataWindow boundary o composition roots. |
| Architecture metrics | `npm run test:architecture:metrics` | Hotspots, budgets de tamaño/imports y allowlist de catálogo/generated. | Bajo-medio | Cambios en composition roots, handler registration, `extension.ts`, `server.ts`, features grandes o catálogo. |
| Docs drift | `npm run test:docs:drift` | Consistencia backlog/current-focus/roadmap/done-log/specs, referencias de prompts y nombres `.prompt.md`. | Bajo | Siempre que cambien docs de estado, backlog, roadmap, prompts o done-log. |
| Performance gate | `npm run test:performance:gate` | Latencia de features críticas, corpus público, batch analysis y artifact `performance-budget-gate.json`. | Alto | Cambios en hot path, cache, parsing, diagnostics, presentation payload o performance docs. |
| Performance soak | `npm run test:performance:soak` | Estabilidad prolongada opt-in y artifacts de sesión. | Muy alto | Antes de release o al tocar scheduler, cache pressure, backpressure o persistencia. |
| Installed VSIX smoke | `npm run test:smoke:installed-vsix` | Activación desde VSIX empaquetado en entorno aislado. | Alto | Antes de release y cuando cambien packaging, bundle o manifest. |
| Release verify | `npm run release:verify` | Carril de release: full tests, architecture rapid, docs drift, performance gate, catálogo, VSIX, contenido, smoke instalada y summary. | Muy alto | Candidato de release o cambios de build/release. |
| Real corpora opcional | `missing: test:real-corpora` | Validación sobre corpora locales PFC/STD/legacy privados con skips honestos. | Variable/muy alto | Sólo opt-in local; hoy se cubre parcialmente por suites performance/smoke con corpus público y fixtures locales gated. No es dependencia obligatoria de CI. |

Reglas de uso:

- No se inventan scripts en instrucciones o prompts: si un lane no existe, se marca como `missing` y se documenta el carril alternativo real.
- El carril de corpora reales nunca debe convertir fixtures privados/locales en requisito obligatorio de CI.
...
El baseline compacto actual de `SYMBOL-QUALITY-01` vive en `test/server/unit/symbolQualityRegressionMatrix.test.ts` y debe seguir cubriendo built-in, user function, event, local/instance/shared/global variable, parameter, inherited, ambiguous, unknown, DataWindow column/property, overlay localizado y completion resolve enrichment con `sourceOrigin`, `confidence`, `reasonCodes`, `i18n fallback` y `payload budget` visibles.

El glosario estable de `SYMBOL-I18N-TERMS-01` vive en `src/server/presentation/terminology.ts` y se valida con `test/server/unit/presentationTerminology.test.ts`, junto con `hoverFormat` y `presentationContracts`, para fijar keys mínimas, fallback `en/es` y labels visibles compartidos sin volver a hardcodes en providers.

El cierre de `SYMBOL-DW-01` se apoya en `test/server/unit/dataWindowFastContext.test.ts`, que ahora fija explícitamente `DataWindow control`, `DataStore`, `DataWindowChild`, `DataObject literal`, `column`, `computed field`, `property path`, `buffer` y `dynamic/unknown binding` sobre `DataWindowFastContext` sin IO ni reanálisis completo.
...
- `test/server/unit/aiCustomizationGovernance.test.ts` fija Bloque 10: `docs/ai-orchestration.md` existe como owner canónico de customizations AI, contratos read-only, context bundles, safe-edit, tools/MCP, validación y seguridad; además todos los prompts ejecutables de `.github/prompts` usan `.prompt.md`.
...
- `test/server/unit/semanticQueryFacade.test.ts`, junto con `queryContext.test.ts`, `semanticQueryService.test.ts`, `hover.test.ts` y `definition.test.ts`, fija el primer slice de Bloque 5: `SemanticQueryFacade` es read-only, reutiliza `KnowledgeBase`/`SemanticQueryService`/`InheritanceGraph`/`SystemCatalog`, expone target symbol, receiver type, callable, inheritance y enum context con confidence/reason/sourceOrigin, y no hace IO ni full parse sobre contexto ya publicado.
...
- `test/server/unit/hoverFormat.test.ts`, `hover.test.ts`, `completion.test.ts`, `signatureHelp.test.ts`, `definition.test.ts`, `diagnostics.test.ts`, `documentAnalysis.test.ts`, `diagnosticsExtra.test.ts`, `featureHandlers.test.ts`, `interactiveHotPathGuards.test.ts`, `interactiveServingPipeline.test.ts`, `presentationContracts.test.ts` y `lspCapabilitiesContract.test.ts` fijan además la auditoría devtools LSP reciente: hover compacto sin metadata interna, completion inicial ligera con `completionItem/resolve`, `CompletionListViewModel`/`CompletionResolveViewModel`, `SymbolHoverViewModel`, `SymbolSignatureViewModel`, `DiagnosticMessageViewModel`, `DefinitionViewModel`, `SemanticTokensViewModel`, definition con key estructurada/stale guard, capabilities reales (`completionProvider.resolveProvider = true`, `codeLensProvider.resolveProvider = false`), descendants custom de DataWindow servidos por jerarquía, `containerSignature` saneado antes del primer `;`, `SD11` sin falsos positivos sobre cierres estructurales, no IO/no workspace scan/no full parse en snapshot caliente y timings interactivos silenciosos salvo primer uso o slow path `>= 50ms`.
...
- `test/server/unit/queryScopePolicy.test.ts` y `featureReadiness.test.ts` fijan la policy v2 de `B266`, con registro único por consumer, readiness/confidence/fallback centralizados, `signatureHelp` dentro del mismo gate declarativo y budgets/payloads explícitos para `hover`, `completion`, `completion-resolve`, `signatureHelp`, `definition`, `references`, `documentSymbols` y `semanticTokens`.
- `test/server/unit/referenceSourcePool.test.ts`, junto con `references.test.ts`, `rename.test.ts`, `queryScopePolicy.test.ts` y `codeLensReferences.test.ts`, fija el pool acotado por proyecto/candidatos, el reuso de `maskedText` en el hot path de `references`/`rename`/CodeLens, el no-widening a `workspace` sin routing de proyecto, la exclusión de `orca-staging/generated` cuando la policy del consumer no los permite y la decisión de no introducir caché final sin evidencia de beneficio.
...
- `test/server/unit/powerbuilderFiles.test.ts`, `test/server/unit/topology.test.ts` y `test/smoke/lsp-guards.extension.test.ts` fijan el guard de borde para markers `.pbw/.pbt/.pbproj/.pbsln` y `.pbl`, comprobando que los fixtures usados por la smoke son markers plausibles consumibles por `parseTopology()` y que aun así no reciben diagnostics ni providers semánticos aunque se fuerce un lenguaje servido por el cliente.
...
- `test/server/unit/architectureImports.test.ts` fija ya el guardrail ampliado de `B228/B277` y Bloques 7-8/12: `knowledge/parsing/utils` no pueden volver a importar `vscode`/`vscode-languageserver`, `client` no puede importar `server`, `runtime/features` no pueden importar `client`, `shared` no puede importar `client/server`, `shared` no puede importar IO/UI/internals runtime, `src/**` no puede depender de `plugin_old/**` por static import, dynamic `import()` o CommonJS `require()`, `build` no puede colgarse del hot path semántico interactivo, `src/server/presentation` no puede importar IO/workspace discovery/parser/stores semánticos runtime y `dataWindow*.ts` sólo puede usar contratos parser ligeros allowlisted; `documentSymbols.test.ts` mantiene verde el mapper de borde.
- `B353` añade `npm run test:architecture:metrics`: `tools/run-architecture-hotspot-guard.mjs` escribe `artifacts/performance/architecture-hotspot-guard.json`, `test/server/unit/architectureImports.test.ts` lo ejecuta dentro de la suite unitaria y el guard fija budgets explícitos para `src/client/extension.ts`, `src/server/server.ts`, `src/client/commandRegistration.ts`, `featureHandlers.ts`, `completion.ts`, `hover.ts`, `signatureHelp.ts`, `definition.ts`, `diagnostics.ts`, `dataWindowFastContext.ts` y `dataWindowServingAdapters.ts`, separando además la allowlist de `generated.generated.ts`, `objectFunctions.ts`, `dataWindowFunctions.ts`, `manual/language/enumerations/index.ts`, `globalFunctions.ts` y `systemEvents.ts`; el reporte publica también `growthPolicy` y sugerencias de extracción para roots/features vigiladas.
...
- `B292` y `BL-06` quedan fijadas por `test/server/unit/powerbuilderParserResilienceFuzz.test.ts` y `test/server/unit/conditionalCompilationGate.test.ts`: el histórico comentado sigue sin contaminar `logicalStatements`, y el detector read-only `conditionalCompilationGate` sólo marca directivas activas fuera de comentarios para mantener el gate de evidencia sin abrir soporte productivo de preprocesador.
...
- `test/server/unit/docsDriftAudit.test.ts` y `npm run test:docs:drift` fijan `B316`: el repo bloquea ítems `Done` todavía activos en backlog, duplicados canónicos en `docs/backlog.md`/`docs/done-log.md`, specs sin `spec.md`/`tasks.md`, prompts `.prompt.md` inexistentes, prompt files con extensión inválida y desalineación entre `docs/current-focus.md` y `docs/roadmap.md`.
...
- `test/server/unit/frameworkKnowledgePacks.test.ts`, `workspaceSymbols.test.ts` y `currentObjectContext.test.ts` cubren además `SYMBOL-FRAMEWORKS-01`: familias PFC/STD mínimas entran como packs advisory, el manifest cae a samples curados cuando el owner framework-specific no existe en el catálogo oficial y el uso de `fixtures-local/pfc`/`fixtures-local/STD_FC_OrderEntry` queda gated con skip honesto.
...
- `catalogLocalization.test.ts`, junto con `catalogConsistency.test.ts`, `catalogAdoptionDecision.test.ts` y `systemCatalogQueryHardening.test.ts`, fija `B371`: el rail `localization/` resuelve overlays documentales por `targetId` o `targetKey` sobre la entry canónica del bucket runtime, mantiene intacto el texto oficial de `generated`, publica un índice español parcial y hace visibles los overlays huérfanos en `buildCatalogConsistencyReport().localization` sin abrir scans globales ni duplicar entries por idioma; desde la schema estricta `1.0.0`, además exige `source` y `reviewed` explícitos en overlays y publica `schemaIssues` si falta metadata obligatoria.
...
- `catalogLocalization.test.ts`, `documentationService.test.ts` y el grep transversal `enum` mantienen cerrado `SYMBOL-CATALOG-ENUMS-ENRICH-P2`: el rail `es` sirve ya overlays revisados de `SaveAsType`, `FillPattern`, `SecureProtocol`, `Text!` y `Primary!`, preserva `obsoleteMessage` cuando la entry canónica lo exige y deja verde la ruta compartida de completion/hover/signatureHelp/diagnostics para contexto enumerado sin traducir anchors técnicos.
- `catalogLocalization.test.ts`, `documentationService.test.ts` y el grep `systemCatalog` mantienen cerrado `SYMBOL-CATALOG-DATATYPES-ENRICH-P2`: el rail `es` sirve ya overlays revisados de `DataStore`, `DataWindowChild`, `Transaction`, `HTTPClient` y `RESTClient`, preserva nombres reales de datatype y deja intacto el runtime base del catálogo mientras sólo cambia documentación visible.
- `catalogLocalization.test.ts`, `completion.test.ts` y el grep `keyword|reserved|catalogLocalization|catalogConsistency` mantienen cerrado `SYMBOL-CATALOG-STATEMENTS-ENRICH-P2`: el rail `es` sirve ya overlays revisados de `IF...THEN`, `CHOOSE CASE`, `FOR...NEXT`, `IF`, `FOR`, `TRUE`, `FALSE` y `NOT`, preserva los lexemas reales y valida que `completion resolve` siga cargando ayuda localizada sin inflar `completion initial`.
...
- `catalogLocalization.test.ts`, `catalogConsistency.test.ts` y el comando `npm run report:catalog-localization` fijan `B374`: el audit de `localization` publica cobertura por dominio, detecta overlays incompletos y anchors técnicos mal anclados (`signatureLabel`/`parameterName` traducidos), agrega `missingFieldsByDomain` para authoring incremental y serializa snapshots deterministas en `artifacts/catalog/` para authoring incremental sin drift.
- `catalogLocalization.test.ts`, `catalogConsistency.test.ts`, `npm run report:catalog-localization` y `npm run migrate:catalog-localization-target-ids` fijan `B375`: overlays con `targetId` obsoleto pero `targetKey` todavía válido se recuperan sin huérfanos silenciosos, el audit publica `recoveredTargetIds`, `reviewed-with-issues` cuando alguien marca como revisado un overlay todavía defectuoso, y el migrador offline deja reconciliación explícita del source sin tocar el hot path interactivo.
...
- `semanticTokens.test.ts` fija ahora `SYMBOL-TOKENS-01`: la taxonomía visible usa sólo token types estándar, publica modifiers explícitos (`defaultLibrary`, `local`, `instance`, `global`) sin depender de locale/textos localizados, separa `class` frente a `type` para tipos de workspace/runtime y congela rangos/modifiers de `parameter`, `local`, `instance`, `shared/global`, built-ins y enum values con `!`; el mismo corte corrige además en [src/server/analysis/documentAnalysis.ts](../src/server/analysis/documentAnalysis.ts) la preservación de `shared/global` inline dentro de `type variables`.
...
Estado actual:
- `hoverFormat.test.ts`, `hover.test.ts`, `definition.test.ts`, `completion.test.ts`, `signatureHelp.test.ts`, `diagnostics.test.ts`, `diagnosticsExtra.test.ts`, `documentSymbols.test.ts`, `workspaceSymbols.test.ts`, `references.test.ts` y `crossSurfaceGoldenMatrix.test.ts` cierran la cadena devtools LSP `DW-01/HOVER-02/HOVER-AUDIT-01`: fijan navegación oficial de built-ins DataWindow sobre descendants custom, ausencia de `workspace fallback` innecesario en hover y una matriz transversal única de surfaces visibles antes de mover el carril a `done-log`.
...
- `test/server/unit/externalFunctions.test.ts`, `rename.test.ts`, `references.test.ts`, `hoverFormat.test.ts`, `diagnostics.test.ts` y `powerBuilderTechnicalDebtReport.test.ts` fijan además que `LIBRARY` y `RPCFUNC` se distinguen en el mismo rail seguro: DLL/PBX mantienen clasificación nativa, `RPCFUNC` se proyecta como stored procedure sin implementación interna, y rename/references siguen degradando sin abrir navegación ficticia.
...
- `test/server/unit/documentAnalysis.test.ts` fija además la frontera `.srd`: stub navegable DataWindow sin secciones, scopes, logical statements ni control blocks del parser PowerScript general.
- `test/server/unit/dataWindowFastContext.test.ts`, junto con `interactiveHotPathGuards.test.ts`, fija el fast mode read-only: receiver kind para DataWindow/DataStore/DataWindowChild, binding high/medium/low, DataObject literal, `GetChild` con confianza limitada, degradación dinámica, columnas sólo con evidencia defendible, property paths seguros, buffers `DWBuffer` desde catálogo y cache caliente sin IO/full parse.
...
- `test/server/unit/hover.test.ts` y `test/server/unit/definition.test.ts` fijan además el primer slice de `BL-05`: columnas literales en `GetItem*` / `SetItemStatus` resuelven sobre el `DataObject` literal y único, incluso con `DWBuffer`, mientras roots dinámicos siguen sin inferencia adicional.
- `test/server/unit/completion.test.ts`, `hover.test.ts`, `definition.test.ts` y `signatureHelp.test.ts` cubren los adapters comunes de DataWindow sobre `DataWindowFastContext` sin reimplementar binding por feature.
...
Estado actual del carril devtools LSP:

- `test/server/helpers/hotPathTestHarness.ts` prepara `KnowledgeBase`, `InheritanceGraph`, `SystemCatalog`, `analysisCache` caliente y spies de IO/full parse para que los tests de serving no dupliquen setup frágil.
- `interactiveHotPathGuards.test.ts` fija `no IO / no workspace scan / no full parse` para `hover`, `completion`, `signatureHelp`, `definition`, `documentSymbols` y `semanticTokens` cuando el snapshot ya está caliente; diagnostics incrementales siguen protegidos por scheduler/analysisCache y quedan fuera de este slice.
- `completionItem/resolve` ya tiene carril unitario: `completion.test.ts` valida lista inicial ligera, documentación localizada diferida, fallback sin `data`, misses reutilizables para negative cache, ranking local/argumento/instancia/built-in, boundaries de comentario/string/receiver desconocido y payload inicial bajo budget; `cacheKeyContract.test.ts` fija segregación por locale/contexto y `interactiveServingPipeline.test.ts` valida métricas, payload budget `completion-resolve` separado de la lista inicial, stale discard y negative-hit que devuelve el item original sin reejecutar provider ni escribir caché final.
- `lspPayloadBudgetContracts.test.ts` mide payloads representativos para `hover`, `completion`, `completion-resolve`, `signatureHelp`, `definition`, `references`, `documentSymbols` y `semanticTokens` contra `src/server/serving/payloadBudget.ts`; la serialización queda confinada a tests.
- `presentationContracts.test.ts` cubre la matriz estructural de Bloque 7: initial/resolve de completion, overlay localizado sin cambio de identidad, diagnostics con code/reason/confidence, semantic tokens ordenados/deduplicados, definition como `null|Location|Location[]` y AI context read-only con policy compacta.
- `SignatureHelpViewModel`, `DefinitionViewModel` con key estructurada/stale guard, no-widening de `references`/`rename`, decisions no-cache de `documentSymbols`/`semanticTokens` y `CodeLens` sin `resolveProvider` quedan cubiertos por las suites focales de Bloques 4 y 7; la deuda abierta pasa a ser sólo medición corpus-driven adicional o cache final cuando exista presión medible.
