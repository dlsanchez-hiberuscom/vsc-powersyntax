...
- `completionItem/resolve` ya tiene carril unitario: `completion.test.ts` valida lista inicial ligera, documentación localizada diferida, fallback sin `data`, ranking local/argumento/instancia/built-in, boundaries de comentario/string/receiver desconocido y payload inicial bajo budget; `interactiveServingPipeline.test.ts` valida métricas, payload budget `completion-resolve` separado de la lista inicial y stale discard que devuelve el item original sin escribir caché.
