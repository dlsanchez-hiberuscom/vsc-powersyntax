...
- `test/server/unit/catalogGeneratorScript.test.ts` fija que el generator oficial apunte al layout actual `src/server/knowledge/system`, que la ruta plural `scripts/generate_official_function_catalog.cjs` quede disponible como wrapper de compatibilidad, que `officialCoverage.generated.ts` publique también `keywords` y `reserved-words`, y que `generatedKeywordLexemes.generated.ts` materialice lexemas oficiales como `commit`, `namespace` y `with`; `catalogV2.test.ts` cubre además `PUBLIC`, `COMMIT` y la alineación efectiva de `PB_KEYWORDS`, mientras mantiene las garantías de `UnsignedInt` y de varios `system-object-datatypes` oficiales generados (`SMTPClient`, `WindowObject`, `PDFAction`, `SyncParm`, `PowerServerResult`) alineados con `PB_BUILTIN_TYPES`.
- `catalogV2.test.ts` fija además el hardening curado de `B324`: `resolveLanguageSymbol('SaveAsType')` debe resolver `SaveAsType!` sin depender del sufijo `!`, y `operators`/`pronouns`/`enumerated-values` no pueden solapar lookup keys con `keywords`/`reserved-words` tras la oficialización de B322.
...
- `completion.test.ts` y `hotPathAllocationBudget.test.ts` fijan `B330`: completion incorpora `reserved-words`, `pronouns`, `system-globals` y `enumerated-values` sólo en contextos globales relevantes, con dedupe/prioridad estable frente a símbolos locales y sin clonar catálogos completos ni mezclar estos dominios en member contexts irrelevantes.
