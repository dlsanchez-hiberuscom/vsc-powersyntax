...
Las smokes `test/smoke/extension.test.ts` y `test/smoke/health-report.extension.test.ts` fijan además `B278`: registro y ejecución read-only del core maintenance command pack, junto con el export real de `health report` bajo `tools/health-reports` o destino explícito.
...
- `test/server/unit/coreMaintenanceCommandCatalog.test.ts`, junto con `test/smoke/extension.test.ts` y `test/smoke/health-report.extension.test.ts`, fija `B278`: los nueve comandos del pack quedan tipados y clasificados entre `read-only` y `confirmable`, el wiring visible permanece estable y el health report exporta dashboard/stats/manifest reales del workspace.
